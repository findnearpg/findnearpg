...
export async function POST(request) {
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (!file) {
        return Response.json({ error: 'Document file is required' }, { status: 400 });
      }
    ...
    const folder = 'findnearpg/verification-documents';
    ...
    if (!response.ok) {
      return Response.json(
        { error: json?.error?.message || 'Failed to upload document to Cloudinary' },
        { status: 500 }
      );
    }
    ...
    console.error('Owner verification document upload failed:', error);
